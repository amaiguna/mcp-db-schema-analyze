...
export function createMcpServer(sqlDirs) {
    ...
    // --- describe-table ---
    server.tool("describe-table", "指定テーブルの詳細(カラム、インデックス、コメント)を返す", { table_name: z.string().describe("テーブル名") }, async ({ table_name }) => {
        const table = await registry.getTable(table_name);
        if (!table) {
            return {
                isError: true,
                content: [{ type: "text", text: `テーブル '${table_name}' が見つかりません` }],
            };
        }
        return { content: [{ type: "text", text: JSON.stringify(table, null, 2) }] };
    });
    // --- list-relations ---
    server.tool("list-relations", "指定テーブルのリレーション(FK確定 + 同名カラム推定)を返す", { table_name: z.string().describe("テーブル名") }, async ({ table_name }) => {
    ...
    // --- list-master-data ---
    server.tool("list-master-data", "マスタデータの一覧、またはテーブル指定でマスタデータの中身を返す", { table_name: z.string().optional().describe("テーブル名 (省略時は一覧を返す)") }, async ({ table_name }) => {
        if (!table_name) {
    ...
    // --- find-shared-columns ---
    server.tool("find-shared-columns", "複数テーブルに存在する同名カラムを検出する (FK未定義のレガシースキーマで暗黙的な関連を発見)", {
        exclude_patterns: z
            .array(z.string())
            .optional()
